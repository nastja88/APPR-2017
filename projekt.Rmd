...
ggplot(alko) + aes(x=leto, y=povprecje) + geom_point() + geom_smooth(method = "lm", formula = y ~ x + I(x^2)) + theme_bw() + labs(y="globalna poraba alkohola (15+) [liter čistega alkohola]")
```
...
ggplot() + geom_polygon(data = left_join(svet, tabela, by=c("sovereignt" = "drzava")), 
                        aes(x = long, y = lat, group = group, fill=dopust)) + theme_void() + ggtitle("Število plačanih prostih dni (5 delovnih dni/teden)") + theme(plot.title = element_text(hjust = 0.5)) + guides(fill=guide_legend(title=NULL))
```
