...
g_mal <- ggplot(filter(bolezni, bolezni$bolezen == "malarija" )) + 
  ...
  geom_boxplot(color = "blue") + theme_bw() + 
  labs(y="število okuženih z malarijo (v milijonih)")

# globalna poraba alkohola (15+)
alko <- filter(znacilnosti, znacilnosti$znacilnost == "alkohol") %>% 
  group_by(leto) %>% summarise(povprecje = mean(pojavnost)) 
...
g_tb <- ggplot(tobak) + aes(x=leto, y=pojavnost, color=slovarcek[spol]) + 
  geom_jitter() + theme_bw() + labs(y="delež kadilcev (15+) [%]") + 
  guides(color = guide_legend(title = "Spol"))
